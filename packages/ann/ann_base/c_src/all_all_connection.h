...
namespace ANN {
  ...
  public:
    ...
    /// This method checks if the size of the input and the
    /// output units is the expected value for this connection.
    bool checkInputOutputSizes(ActivationUnits *input,
			       ActivationUnits *output) const;

    /// This method initializes the weights of the connection.
    void randomizeWeights(MTRand *rnd, float low, float high, bool use_fanin);

    /// This method initializes the weights of a column in the connection.
    ...
    /// This method loads the weights of the connection from a matrix.
    unsigned int loadWeights(MatrixFloat *data,
			     MatrixFloat *old_data,
			     unsigned int first_weight_pos,
			     unsigned int column_size);

    /// This method saves the weights of the connection to a matrix.
    unsigned int copyWeightsTo(MatrixFloat *data,
			       MatrixFloat *old_data,
			       unsigned int first_weight_pos,
			       unsigned int column_size);

    /// This method returns a copy of this connection.
    Connections *clone();
  };
}
