...
namespace ANN {
  ...
  void MaxPoolingANNComponent::initializeArrays(const int *input_dims) {
    ...
    for (int i=0; i<input_num_dims; ++i) {
      output_dims[i+1] = (input_dims[i+1] - kernel_dims[i])/kernel_step[i];
      
      input_window_size[i+1]	   = kernel_dims[i];
      input_window_num_steps[i+1]  = output_dims[i+1];
  ...
  MaxPoolingANNComponent::MaxPoolingANNComponent(int input_num_dims,
    ...
    kernel_dims(new int[input_num_dims]),
    kernel_step(new int[input_num_dims]),
    ...
    for(int i=0; i<input_num_dims; ++i) {
      ...
      kernel_dims[i] = _kernel_dims[i];
      kernel_step[i] = _kernel_step[i];
  ...
  Token *MaxPoolingANNComponent::doForward(Token *_input, bool during_training) {
    ...
    while(!input_sw.isEnd() && !output_sw.isEnd()) {
      ...
      IncRef(max_sel_dim);
      MatrixFloat *max_sel_dim_rewrapped = max_sel_dim->rewrap(max_rewrap,
							       input_num_dims+1);
  ...
  // The MaxPoolingANNComponent
  void MaxPoolingANNComponent::doUpdate() {
    /*
  ...
  void MaxPoolingANNComponent::setOption(const char *name, double value) {
  ...
  bool MaxPoolingANNComponent::hasOption(const char *name) {
  ...
  double MaxPoolingANNComponent::getOption(const char *name) {
  ...
  void MaxPoolingANNComponent::build(unsigned int _input_size,
  ...
  void MaxPoolingANNComponent::copyWeights(hash<string,Connections*> &weights_dict) {
    if (weights_matrix == 0)
