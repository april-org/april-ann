...
Image<T>::Image(Matrix<T> *mat, 
  ...
  matrix = new Matrix<T>(mat, pos, sizes);
  IncRef(matrix);
}
...
Image<T>* Image<T>::clone_subimage(int w, int h,
  ...
  dims[0] = h;
  dims[1] = w;
  Matrix<T> *mat = new Matrix<T>(2, dims, matrix->getMajorOrder());
...
int Image<T>::count_black_pixels(T threshold) const{
  int count = 0;
  typename Matrix<T>::random_access_iterator mat_it(matrix);
...
void Image<T>::projection_v(T *v) const {
  ...
  for (int x=0;x<width();x++)
    v[x] = 0;
  typename Matrix<T>::const_iterator mat_it(matrix->begin());
...
void Image<T>::projection_v(Matrix<T> **m) const {
  int dims[1];
  dims[0] = width();
  *m = new Matrix<T>(1, dims, matrix->getMajorOrder());
...
Image<T>* Image<T>::shear_h(double angle, T default_value) const {
  ...
  if (angle > 0) {
    // Angle > 0
    typename Matrix<T>::const_iterator source_it(matrix->begin());
    typename Matrix<T>::iterator dest_it(mat->begin());
    ...
    for (int line= 0 ; line < height(); line++){
      typename Matrix<T>::const_iterator source_it(matrix->iteratorAt(height()-line-1,0));
      typename Matrix<T>::iterator dest_it(mat->iteratorAt(height()-line-1,0));
...
void Image<T>::min_bounding_box(float threshold,
  ...
  found = false;
  row = 0;
  typename Matrix<T>::const_random_access_iterator mat_it(matrix);
...
Image<T> *Image<T>::convolution5x5(float *k, T default_color) const
{
  ...
  Matrix<T> *result_mat = conv_result_mat->rewrap(rewrap_dims, 2);
