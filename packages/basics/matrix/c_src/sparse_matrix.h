...
/// the matrices is inmutable, but not the content values. Therefore, if you
/// want to add new non-zero elements, a new SparseMatrix will be allocated,
/// with enough size to store the old SparseMatrix non-zero elements plus the
/// new elements. It is not possible to share internal memory pointers between
/// different matrices.
template <typename T>
...
protected:
  ...
  GPUMirroredMemoryBlock<int> *indices;     ///< indices for rows (CSC) or columns (CSR)
  GPUMirroredMemoryBlock<int> *first_index; ///< size(values) + 1
  GPUMirroredMemoryBlock<int> *ptrB; ///< first indices (points to first_index)
  GPUMirroredMemoryBlock<int> *ptrE; ///< last indices (points to first_index+1)
