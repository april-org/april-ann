...
matrixInt32.meta_instance.__tostring = function(self)
  local dims   = self:dim()
  local coords = {}
  local out    = {}
  local row    = {}
  ...
  for i=1,self:size() do
    ...
    table.insert(row, string.format("% 11d", self:get(table.unpack(coords))))
    local j=#dims+1
    repeat
      j=j-1
      coords[j] = coords[j] + 1
  ...
  table.insert(out, string.format("# MatrixInt32 of size [%s] [%s]",
				  table.concat(dims, ","),
