...
check(function() return cov:equals(aU * aS:to_dense("col_major") * aVT, 1e-04 ) end)
...
local aUmul = aU:clone():gemm{ A=aU, B=aU, trans_B=true, alpha=1.0, beta=0.0, }
check(function() return aUmul:equals( aUmul:clone():zeros():diag(1), 0.001 ) end)
-- V matrix orthogonality check
local aVTmul = aVT:clone():gemm{ A=aVT, B=aVT, trans_B=true, alpha=1.0, beta=0.0, }
check(function() return aVTmul:equals( aVTmul:clone():zeros():diag(1), 0.001 ) end)

-- check U matrix with octave computation
local refU = matrix.fromTabFilename(base_dir.."data/U.gz", "col_major"):abs()
check(function() return refU:equals(aU:clone():abs(), 0.001) end)

-- check V matrix with octave computation
local refV = matrix.fromTabFilename(base_dir.."data/V.gz", "col_major"):transpose():abs()
...
check(function() return refS:diagonalize():equals(aS:to_dense("col_major"), 0.001) end)
...
-- U matrix orthogonality check
local bPmul = bP:clone():gemm{ A=bP, B=bP, trans_B=true, alpha=1.0, beta=0.0, }
check(function() return bPmul:equals( bPmul:clone():zeros():diag(1), 1e-03 ) end)
