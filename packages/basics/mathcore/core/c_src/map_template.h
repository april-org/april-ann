...
#define DECL_MAP1_FUNCTOR(NAME,FUNC)                    \
  template<typename T, typename O>                      \
  struct NAME {                                         \
    APRIL_CUDA_EXPORT O operator()(const T &v) {        \
      return FUNC(v);                                   \
    }                                                   \
  }

#define HEADER_MAP1_WRAPPER(NAME)                                   \
  template<typename T, typename O>                                  \
  void   NAME(const GPUMirroredMemoryBlock<T> *input,               \
...
#define DECL_MAP1_WRAPPER(NAME,F)                                  \
  template<typename T, typename O>                                  \
  void   NAME(const GPUMirroredMemoryBlock<T> *input,               \
              unsigned int input_stride,                            \
              unsigned int input_shift,                             \
...
#define INSTANTIATE_TEMPLATE_MAP1_WRAPPER(NAME,TYPE_T,TYPE_O)           \
    template void  NAME<TYPE>(const GPUMirroredMemoryBlock<TYPE_T> *input, \
                              unsigned int input_stride,                \
                              unsigned int input_shift,                 \
                              GPUMirroredMemoryBlock<TYPE_O> *output,   \
                              unsigned int output_stride,               \
                              unsigned int output_shift,                \
                              unsigned int N,                           \
                              bool use_gpu)

#define DECL_MAP2_FUNCTOR(NAME,FUNC)                              \
  template<typename T1, typename T2, typename O>                  \
  struct NAME {                                                     \
    APRIL_CUDA_EXPORT O operator()(const T1 &a, const T2 &b) {      \
      return FUNC(a, b);                                            \
    }                                                               \
  }

#define HEADER_MAP2_WRAPPER(NAME)                                       \
  template<typename T1, typename T2, typename O>                        \
    void   NAME(const GPUMirroredMemoryBlock<T1> *input1,               \
                unsigned int input1_units_stride,                       \
                unsigned int input1_units_shift,                        \
                const GPUMirroredMemoryBlock<T2> *input2,               \
                unsigned int input2_errors_stride,                      \
                unsigned int input2_errors_shift,                       \
                GPUMirroredMemoryBlock<O> *output_errors,               \
                unsigned int output_errors_stride,                      \
                unsigned int output_errors_shift,                       \
                unsigned int N,                                         \
                bool use_gpu);

#define DECL_MAP2_WRAPPER(NAME,F)                                      \
  template<typename T1, typename T2, typename O>                        \
  void   NAME(const GPUMirroredMemoryBlock<T1> *input1,                 \
              unsigned int input1_units_stride,                         \
              unsigned int input1_units_shift,                          \
...
#define INSTANTIATE_TEMPLATE_MAP2_WRAPPER(NAME,TYPE_T1,TYPE_T2,TYPE_O)  \
  template void  NAME<TYPE>(const GPUMirroredMemoryBlock<TYPE_T1> *input1, \
                            unsigned int input1_units_stride,           \
                            unsigned int input1_units_shift,            \
                            const GPUMirroredMemoryBlock<TYPE_T2> *input2, \
                            unsigned int input2_errors_stride,          \
                            unsigned int input2_errors_shift,           \
                            GPUMirroredMemoryBlock<TYPE_O> *output_errors, \
                            unsigned int output_errors_stride,          \
                            unsigned int output_errors_shift,           \
                            unsigned int N,                             \
                            bool use_gpu)
