// Bedaux.cpp
// Test output and speed of Bedaux's Mersenne Twister class
...
{
	printf( "Testing output and speed of Bedaux's Mersenne Twister class\n" );
	...
	printf( "\nTest of random integer generation:\n" );
	MTRand_int32 mtrand1( bigSeed, 4 );
	unsigned long i;
	for( i = 0; i < 1000UL; ++i )
	{
		printf( "%10lu ", mtrand1() );
	...
	MTRand mtrand2( bigSeed, 4 );
	for( i = 0; i < 1000UL; ++i )
		mtrand2();  // advance to the proper position in the series
	for( i = 0; i < 1000UL; ++i )
	{
		printf( "%10.8f ", mtrand2() );
		if( i % 5 == 4 ) printf("\n");
	}
	
	printf( "\nTest of random real number [0,1] generation:\n" );
	MTRand_closed mtrand3( oneSeed );
	for( i = 0; i < 2000UL; ++i )
	{
		printf( "%10.8f ", mtrand3() );
		if( i % 5 == 4 ) printf("\n");
	}
	
	printf( "\nTest of time to generate one billion random integers:\n" );
	MTRand_int32 mtrand4( oneSeed );
