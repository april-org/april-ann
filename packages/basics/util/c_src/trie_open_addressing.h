 ...
 *
 */
#ifndef TRIE_OPEN_ADDR_H
#define TRIE_OPEN_ADDR_H
...
namespace april_utils {
  ...
  public:
    ...
    typedef april_utils::open_addr_hash_timestamp<int_pair, int> transitions_t;
    
  private:
    // tabla de finales
    april_utils::open_addr_hash_timestamp<int, O> outputs_tbl;
  ...
  public:
    ...
    // iterador sencillo
    class iterator {
      friend class TrieOpenAddr;
      TrieOpenAddr *trie;
      typename transitions_t::iterator hash_it;
      
      iterator(TrieOpenAddr *trie, typename transitions_t::iterator it) {
	this->trie    = trie;
	this->hash_it = it;
      }
    ...
    TrieOpenAddr(int num_nodes = 8) :
      outputs_tbl(8, 0.6),
      transitions_tbl(8, 0.6)
    {
    ...
    /// Inserta una secuencia en trie, y le asocia value como salida
    void insert(const int *key, int last_key, O value, int key_size) {
      int current_node = 0;
      int next_level = 1;
      for (int i=0; i<key_size; ++i) {
	int_pair tr_key(current_node, key[i]);
	...
	next_level++;
      ...
      {
	int_pair tr_key(current_node, last_key);
	// busca la componente actual de la secuencia en el estado current_node
	const int *aux = transitions_tbl.find(tr_key);
	if (aux != 0) current_node = *aux;
	else {
    ...
    /// key_size, y si la encuentra, y es un nodo de salida, devuelve el
    /// valor tipo O guardado en dicho nodo.
    const O *find(const int *key, int last_key, int key_size) {
      int current_node = 0;
      // iteramos sobre el tamanyo de key
      for (int i=0; i<key_size; ++i) {
	const int *aux = transitions_tbl.find(int_pair(current_node, key[i]));
	...
	current_node = *aux;
      ...
      {
	const int *aux = transitions_tbl.find(int_pair(current_node, last_key));
	if (aux == 0) return 0;
	current_node = *aux;
      }
      // buscamos el nodo en la lista de outputs del Trie
