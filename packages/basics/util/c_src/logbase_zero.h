 ...
 *
 */
#ifndef LOGBASE_ZERO_H
#define LOGBASE_ZERO_H
...
class log_float {
 ...
 public:
  ...
  log_float raise_to(float exponent) {
    ...
    if (aux <= rawscorezero) aux = rawscorezero;
  ...
  log_float operator += (const log_float &y) {
    *this = *this + y;
    if (raw_value <= rawscorezero)
      raw_value = rawscorezero;
    return *this;
  }
...
inline log_float operator + (const log_float &x, const log_float &y) {
  ...
  return (result <= log_float::zero()) ? log_float::zero() : result;
}
inline log_float operator - (const log_float &x, const log_float &y) {
