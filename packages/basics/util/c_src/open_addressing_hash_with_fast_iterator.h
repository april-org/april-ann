 ...
 *
 */
#ifndef OPEN_ADDR_HASH_FAST_IT_H
#define OPEN_ADDR_HASH_FAST_IT_H
...
namespace april_utils {
  ...
  public:
    int *iterator_vector; // vector para poder iterar de forma
			  // eficiente sobre los elementos de la tabla
  private:
    //
    ...
    EqualKey equal_key;
  public:
    open_addr_hash_fast_it(int nbckts=8, float mxloadf=0.75); // constructor
    open_addr_hash_fast_it(const open_addr_hash_fast_it&);   // copy constructor
    ~open_addr_hash_fast_it(); // destructor
    open_addr_hash_fast_it& operator=(const open_addr_hash_fast_it&);  // assignment operator
    ...
    class iterator {
      ...
      friend class open_addr_hash_fast_it;

      open_addr_hash_fast_it *h;
      int index;

      iterator(open_addr_hash_fast_it *h, int index):
	h(h), index(index) {}
	...
	//
	return h->buckets[h->iterator_vector[index]].value;
	//
      }
    ...
    class const_iterator {
      ...
      const open_addr_hash_fast_it *h;
      int index;
        
      const_iterator(open_addr_hash_fast_it *h, int index):
	h(h), index(index) {}
  ...
  public:
    iterator& end() {
      //end_iterator = iterator(this, the_size);
      return end_iterator;
    }
  ...
  int open_addr_hash_fast_it<ky,dt,hfcn,eqky>::first_power_of_2(int value) {
  ...
  template <typename ky, typename dt, 
	    typename hfcn, typename eqky>
  open_addr_hash_fast_it<ky,dt,hfcn,eqky>::open_addr_hash_fast_it(int nbckts, float mxloadf) :
    end_iterator(this, 0), the_size(end_iterator.index) {
    ...
    //    end_iterator    = iterator(this, 0);
    current_stamp   = 1;
    buckets         = new node[num_buckets];
    for (int i=0; i<num_buckets; i++)
      buckets[i].stamp = 0;
    //
    iterator_vector = new int[rehash_threshold+1];
    //
  }

  template <typename ky, typename dt, 
	    typename hfcn, typename eqky>
  open_addr_hash_fast_it<ky,dt,hfcn,eqky>::open_addr_hash_fast_it(const open_addr_hash_fast_it& other) :
    end_iterator(this, other.the_size), the_size(end_iterator.index) { // copy constructor
  ...
  template <typename ky, typename dt, 
	    typename hfcn, typename eqky>
  void open_addr_hash_fast_it<ky,dt,hfcn,eqky>::clear() {
    ...
    the_size = 0;
    // ESTO NO HACE FALTA, se hace al llamar a end():
    // end_iterator    = iterator(this, 0);
  }

  template <typename ky, typename dt, 
    typename hfcn, typename eqky>
  open_addr_hash_fast_it<ky,dt,hfcn,eqky>& 
  open_addr_hash_fast_it<ky,dt,hfcn,eqky>::operator=(const open_addr_hash_fast_it& other) {  // The assignment operator
    if (this != &other) {
      hash_mask       = other.hash_mask;
      //      the_size        = other.the_size;
      max_load_factor = other.max_load_factor;
	...
	buckets      = new node[num_buckets];
	//
	delete[] iterator_vector;
	iterator_vector = new int[rehash_threshold+1];
	//
      }
      for (int i=0; i<num_buckets; i++)
	buckets[i] = other.buckets[i];
      end_iterator    = iterator(this, other.the_size);
    }
    return *this;
  }
  ...
  template <typename ky, typename dt, 
	    typename hfcn, typename eqky>
  open_addr_hash_fast_it<ky,dt,hfcn,eqky>::~open_addr_hash_fast_it() { // destructor
    delete[] buckets;
    //
    delete[] iterator_vector;
    //
  ...
  template <typename ky, typename dt, 
	    typename hfcn, typename eqky>
  dt* open_addr_hash_fast_it<ky,dt,hfcn,eqky>::find(const ky& k) {
  ...
  template <typename ky, typename dt, 
	    typename hfcn, typename eqky>
  const pair<const ky,dt> * open_addr_hash_fast_it<ky,dt,hfcn,eqky>::find_pair(const ky& k) const {
  ...
  template <typename ky, typename dt, 
	    typename hfcn, typename eqky>
  dt& open_addr_hash_fast_it<ky,dt,hfcn,eqky>::operator[] (const ky& k) {
    unsigned int hfval = hash_function(k);
    unsigned int index = hfval & hash_mask;
    unsigned int step  = (hfval >> 15) | 1; // debe ser impar
    while (buckets[index].stamp == current_stamp) {
    ...
    //
    iterator_vector[the_size-1] = index;
    //
    buckets[index].stamp       = current_stamp;
    *const_cast<ky*>(&buckets[index].value.first) = k;
    return buckets[index].value.second;
  }

  template <typename ky, typename dt, 
	    typename hfcn, typename eqky>
  void open_addr_hash_fast_it<ky,dt,hfcn,eqky>::resize(int n) {
	...
	if (old_buckets[i].stamp == old_stamp) {
	  ...
	  *const_cast<ky *>(&buckets[index].value.first) = old_buckets[i].value.first;
	  ...
	  //
	  iterator_vector[it_vector_pos]		 = index;
	  it_vector_pos++;
	  //
	}
      delete[] old_buckets;
