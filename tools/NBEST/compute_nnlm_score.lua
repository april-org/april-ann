...
while true do
  ...
  local key
  if use_bcc then
    for i=1,lm:getLMOrder()-1 do
      table.insert(context, vocab:getWordId(begin_word))
    end
    key = lm:getInitialKey(vocab:getWordId(begin_word))
  else
    for i=1,lm:getLMOrder()-1 do
      table.insert(context, 0)
    end
    key = lm:getZeroGramKey(vocab:getWordId(begin_word))
  end
  for wpos=1,#wids do
    ...
    -- next key
    key = lm:findKeyFromNgram(context)
  end
  if use_ecc then
    ngrams[key] = ngrams[key] or {}
